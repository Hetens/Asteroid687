...
class SimpleLogger(BaseCallback):
    def __init__(self, csv_path):
        ...
        with open(csv_path, 'w', newline='') as f:
            csv.writer(f).writerow(['episode', 'return', 'length', 'critic_loss'])
    ...
    def _on_step(self):
        ...
        for info in infos:
            if 'episode' in info:
                ...
                if self.last_ep_count % 50 == 0:
                    recent_mean = np.mean(self.returns[-50:])
                    recent_max = max(self.returns[-50:])
                    print(f"Ep {self.last_ep_count}: Mean={recent_mean:.1f}, Max={recent_max:.1f}")
        return True
...
def main():
    ...
    print(f"\n🚀 Starting training with ANTI-WALL-HUGGING rewards...")
    ...
    if logger.returns:
        ...
        plt.subplot(1, 2, 1)
        plt.plot(logger.returns, alpha=0.3)
        if len(logger.returns) > 50:
            smooth = np.convolve(logger.returns, np.ones(50)/50, mode='valid')
            plt.plot(range(49, len(logger.returns)), smooth, 'r', linewidth=2)
        plt.xlabel('Episode')
        plt.ylabel('Return')
        ...
        print(f"Final mean (last 100): {np.mean(logger.returns[-100:]):.1f}")
